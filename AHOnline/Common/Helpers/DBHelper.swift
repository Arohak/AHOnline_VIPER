...
struct DBHelper {
    ...
    static let config = Realm.Configuration(
        schemaVersion: Config.getBundleVersionNumber(),
    ...
    static func updateUser(userInfo: UserInfo) {
        ...
        favoriteProduct(products: userInfo.favorites)
    }
    ...
    static func getUserCart() -> User? {
        ...
        try! realm.write {
            ...
            for product in DBHelper.getOrders() {
    ...
    static func storeProduct(product: Product) {
        try! realm.write {
            ...
            let value = getOrderCounts()
            Wireframe.setBadge(value)
        }
    }
    ...
    static func addProduct(product: Product) {
        ...
        if let findProduct = findProduct {
            ...
            updateOrder(product: findProduct, count: count)
        } else {
            let count = product.countBuy + 1
            storeProduct(product: product)
            updateOrder(product: product, count: count)
        }
    }
    
    static func updateCartInfo(mobileNumber: String, address: String, city: String, alias: String, deliveryPrice: Double, date: Date, payment: String)  {
        if let user = getUser() {
            try! realm.write {
                let cart = user.cart!
    ...
    static func removeOrder(product: Product) {
        if let user = getUser() {
            try! realm.write {
                product.countBuy = 0
                user.cart.ordersTotalPrice = getOrdersTotalPrice()
                user.cart.products.remove(at: user.cart.products.index(of: product)!)
