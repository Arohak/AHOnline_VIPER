...
extension CartPresenter: CartViewOutput {
    ...
    func addOrder() {
        router.selectTabControllerForIndex(index: 1)
    }
    
    func updateCart(phone mobileNumber: String, address: String, city: String, alias: String, deliveryPrice: Double, date: Date, payment: String) {
        interactor.updateCartInfo(phone: mobileNumber, address: address, city: city, alias: alias, deliveryPrice: deliveryPrice, date: date, payment: payment)
    }
    
    func updateOrder(product: Product, count: Int) {
        interactor.updateOrder(product: product, count: count)
    }
    
    func removeOrder(product: Product) {
        interactor.removeOrder(product: product)
    }
